...
export class AirportComponent implements OnInit {
  ...
  airport = new FormGroup({
    ...
    lat: new FormControl('', [Validators.required, Validators.min(-90)]),
    lon: new FormControl('', [Validators.required, Validators.min(-90)]),
    apPax: new FormControl('', [Validators.required, Validators.min(0)]),
    apNo: new FormControl('', [Validators.required, Validators.min(0)]),
    ctfPax: new FormControl('', [Validators.required, Validators.min(0)]),
    ctfTeam: new FormControl('', [Validators.required, Validators.min(0)]),
    ctfProtect: new FormControl(false),
    ratingMin: new FormControl('', [Validators.required, Validators.min(0)]),
    ratingMax: new FormControl('', [Validators.required, Validators.min(0)]),
    apPaxBL: new FormControl('', [Validators.required, Validators.min(0)]),
    sdRatio: new FormControl('', [Validators.required, Validators.min(0)]),
    sdAct: new FormControl('', [Validators.required, Validators.min(-999999999)]),
  ...
  constructor(private airportService: AirportsService, private alertService: AlertService, private route: ActivatedRoute, private router: Router, private _modalService: NgbModal) {
    ...
    if (!this.id) {
      ...
      router.navigate(['/operations/data/airports']);
      return;
    }
  ...
  submitAirport() {
    if (!this.id || !this.airport.valid) {
      ...
      this.router.navigate(['/operations/data/airports']);
      return;
    }
